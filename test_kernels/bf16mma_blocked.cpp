...
static constexpr int mma_m = 4; 
static constexpr int mma_n = 4; 
static constexpr int mma_k = 4; 
static constexpr int wavefront_size = 64;
static constexpr int 
//this one tv layouts are as follows: 
//A: looks like A column vector of 32 threads, each thread loads a value fragment of a row vector of size 4. 
//B looks like a row of 32 threads, each thread loads a column vector of size 4. 
//C looks like B x4?
...
__global__ void matmul(const bf16_t* A, const bf16_t* B, float* C)
{
  ...
  int a_row = l % 32; 
  int a_col_thread = (l / 32) * 4; 
  
  int b_row_thread = (l / 32) * 4; 
  int b_col = l % 32; 
  ...
  c_reg = __builtin_amdgcn_mfma_f32_32x32x8bf16_1k(a_reg, b_reg, c_reg, 0, 0, 0);
